...
The CBCT sector is generated by `cbct_sector_zoom(...)`, which samples the CBCT directly onto
the real US display fan (`reslice.us_display.render_on_us_grid`, fitted once by
`calib/fit_us_fan.py` into `calib/us_fan.json`). This makes the CBCT pixel-aligned with the
paired US by construction -- the earlier display path bbox-cropped and stretched each sector to
the US size (support IoU ~0.30); see `figures/10_fan_geometry_mismatch/`. If the fan JSON is
missing it is fitted here from `--ref-sequence` / `--us-spacing` and saved.
...
def _ensure_us_fan(ref_sequence: Path, us_spacing_mm: float) -> None:
    """Guarantee ``calib/us_fan.json`` exists; fit it from the reference sequence if not.

    The fitted fan is the single source of truth for the US display geometry that
    ``cbct_sector_zoom`` samples the CBCT onto. Fitting here (when absent) is what finally
    connects ``--ref-sequence`` / ``--us-spacing`` to the pairs -- previously they were unused.
    """
    from reslice.us_display import DEFAULT_FAN_JSON

    if DEFAULT_FAN_JSON.exists():
        return
    from calib.fit_us_fan import fit

    if not ref_sequence.exists():
        raise SystemExit(
            f"no US fan calibration at {DEFAULT_FAN_JSON} and --ref-sequence {ref_sequence} "
            f"is missing; run calib/fit_us_fan.py first"
        )
    fan, _ = fit(ref_sequence, us_spacing_mm)
    fan.save(DEFAULT_FAN_JSON)
    print(f"fitted US display fan from {ref_sequence.name} -> {DEFAULT_FAN_JSON}")
        ...
        rows.append(
            {
                ...
                "cbct_source": "reslice.us_display.render_on_us_grid (via cbct_sector_zoom)",
...
def main() -> None:
    ...
    _ensure_us_fan(args.ref_sequence, args.us_spacing)
