...
def cbct_sector_zoom(
    ...
    *,
    ...
    ``crop_margin_px`` and ``crop_near_mm`` change only the final crop window.  Pose
    conversion, plane sampling, surface/apex detection and sector geometry remain identical.
    ...
    crop_mask = sec.sector_mask_in_display_image(
        ...
        dbg["fov_deg"], dbg["depth_mm"], float(crop_near_mm),
        dbg["mm_per_row"], dbg["mm_per_col"],
    )
    zoom, _ = sec.crop_and_zoom_sector(
        sec.normalize_image(sector), mask, crop_mask, tuple(target_shape),
        margin_px=crop_margin_px,
        margin_rows_px=crop_margin_rows_px,
        margin_cols_px=crop_margin_cols_px,
    )
    return zoom
